...
export async function GET(req: NextRequest) {
  ...
  // 60 days back, 60 days forward (120-day range confirmed working with CourtReserve)
  const from = new Date();
  ...
  try {
    const url = `https://api.courtreserve.com/api/v1/eventcalendar/eventlist?orgId=${ORG_ID}&startDate=${fmt(from)}&endDate=${fmt(to)}`;
